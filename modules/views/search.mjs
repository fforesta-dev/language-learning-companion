...
function renderNoSearch(root) {
    root.innerHTML = `
    <div class="card">
      <h3>No search term provided</h3>
      <p class="meta">Use the search box above to look up a word.</p>
    </div>
  `;
}

function renderSearchResult(root, data, thesaurus) {
    ...
    root.innerHTML = `
    <div class="daily-word-container" aria-label="Search results">
      <article class="card daily-word-main" aria-label="Search result">
        <h2>Search Result</h2>
...
export async function renderSearch(viewRoot, params) {
    const wordParam = (params.get("word") || "").trim();

    if (!wordParam) {
        renderNoSearch(viewRoot);
        return;
    }
    ...
    try {
        const [dictResult, thesResult] = await Promise.allSettled([
            getWordData(wordParam),
            getThesaurusData(wordParam),
        ]);
        ...
        renderSearchResult(viewRoot, data, thesaurus);
    } catch (err) {
